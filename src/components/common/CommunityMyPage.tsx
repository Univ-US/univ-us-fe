...
const SAMPLE_MY_COMMENTS: MyComment[] = [
  {
    ...
    postTitle: '중간고사 끝나고 다 같이 MT 갈 사람?',
    board: '자유',
    createdAt: '30분 전',
  },
  {
    ...
    postTitle: '도서관 3층 콘센트 자리 명당 공유합니다',
    board: '자유',
    createdAt: '어제',
  },
];
...
const MENU: {
  ...
  },
  {
    ...
    items: [
      {
        key: 'profile',
        icon: <UserRoundCog className='size-[17px]' />,
        label: '프로필 수정',
      },
      {
        key: 'account',
...
function MyWishlist({ wishlist }: { wishlist: MyWishlist[] }) {
  return (
    <>
      <SectionTitle sub={`찜한 상품 ${wishlist.length}개`}>
        관심목록
      </SectionTitle>
      ...
      <div className='max-w-[560px] overflow-hidden rounded-2xl border border-border bg-white p-6 shadow-sm'>
        ...
        <div className='mb-5'>
          ...
          <input
            ...
            className='flex h-11 w-full rounded-xl border border-input bg-background px-3.5 text-[14px] outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20'
          />
        ...
        <div className='mb-6'>
          ...
          <textarea
            ...
            rows={3}
            className='w-full rounded-xl border border-input bg-background px-3.5 py-3 text-[14px] leading-relaxed outline-none transition-colors focus:border-primary focus:ring-2 focus:ring-primary/20'
          />
        </div>
        ...
        <div className='flex items-start gap-5'>
          <div className='w-[220px] shrink-0 overflow-hidden rounded-2xl border border-border bg-white py-2 shadow-sm'>
            {MENU.map((group, gi) => (
              <div
                key={gi}
                className={cn(gi > 0 && 'border-t border-border mt-1 pt-1')}
              >
                <div className='px-[18px] pb-1.5 pt-3 text-[11px] font-bold uppercase tracking-widest text-slate-400'>
                  {group.group}
                </div>
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'flex w-full items-center gap-3 border-r-2 px-[18px] py-2.5 text-left text-[13px] font-semibold transition-all',
                        active
          ...
          <div className='min-w-0 flex-1'>{blocks[section]}</div>
        </div>
      </div>
    </div>
  );
